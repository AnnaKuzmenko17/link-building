...
import { removeFromCartAction, createOrdersAction } from './actions'
...
export function CartClient({ cartItems }: Props) {
  ...
  const allSelected = cartItems.length > 0 && cartItems.every((item) => !!months[item.id])
  ...
  if (cartItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-16 text-center">
        ...
        <Button variant="link" className="mt-2" onClick={() => router.push('/dashboard/client/sites')}>
          Browse sites
        </Button>
      </div>
    )
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <div key={item.id} className="flex flex-col gap-3 rounded-lg border p-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Select
                ...
                onValueChange={(v) => setMonths((prev) => ({ ...prev, [item.id]: v ?? '' }))}
      ...
      <div className="flex justify-end">
        <Button onClick={handleCreateOrders} disabled={!allSelected || isPending}>
