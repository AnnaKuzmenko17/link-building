...
import { siteSchema, type SiteFormValues } from "./types";
...
export async function changeSiteStatusAction(
  ...
  if (action === "request_changes" && status !== "pending") {
    return {
      success: false,
      error: "Can only request changes on a pending site.",
    };
  }
  if (
    action === "approve" &&
    ...
    return {
      success: false,
      error: "Can only approve a pending or needs-changes site.",
    };
  }
  ...
  if (error)
    return {
      success: false,
      error: "Failed to update site status. Please try again.",
    };
