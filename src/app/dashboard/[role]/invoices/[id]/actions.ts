...
const updateSchema = z.object({
  ...
  items: z.array(
    z.object({ id: z.string().uuid(), amount: z.number().positive() })
  ),
});
...
export async function updateInvoiceAction(
  ...
  const { error: fieldsError } = await updateInvoiceFields(
    supabase,
    parsedId.data,
    {
  ...
  for (const item of parsed.data.items) {
    const { error } = await updateInvoiceItemAmount(
      supabase,
      item.id,
      item.amount
    );
...
export async function markAsPaidAction(invoiceId: string): Promise<Result> {
  ...
  if (invoice.status !== "sent")
    return { success: false, error: "Invoice is not in sent status." };

  const { error: statusError } = await setInvoiceStatus(
    supabase,
    parsedId.data,
    "paid"
  );
  ...
  const { error: ordersError } = await updateOrdersToCompleted(
    supabase,
    orderIds
  );
  if (ordersError)
