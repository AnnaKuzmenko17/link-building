...
import { chatIdSchema, CreateResult, SearchResult } from "./types";
...
export async function sendMessageAction(
  ...
  if (chat.status === "archived")
    return { success: false, error: "This chat is archived." };
...
export async function markMessagesReadAction(chatId: string): Promise<Result> {
  ...
  revalidatePath(`/dashboard/${role}`, "layout");
  return { success: true };
}
...
export async function createChatAction(
  ...
  if (allParticipants.length < 2)
    return { success: false, error: "Select at least one other participant." };
...
export async function editChatAction(
  ...
  const chat = await getChatById(supabase, parsedId.data, user.id);
  if (!chat) return { success: false, error: "Chat not found." };
  if (chat.category !== "general")
    return { success: false, error: "Only Standard chats can be edited." };
...
export async function archiveChatAction(chatId: string): Promise<Result> {
  ...
  if (!chat) return { success: false, error: "Chat not found." };
  if (chat.category !== "general")
    return { success: false, error: "Only Standard chats can be archived." };
  if (chat.status === "archived")
    return { success: false, error: "Chat is already archived." };
...
export async function unarchiveChatAction(chatId: string): Promise<Result> {
  ...
  if (!chat) return { success: false, error: "Chat not found." };
  if (chat.category !== "general")
    return { success: false, error: "Only Standard chats can be unarchived." };
  if (chat.status === "active")
    return { success: false, error: "Chat is already active." };
...
export async function startOrderChatAction(
  orderId: string
): Promise<CreateResult> {
  const { user } = await requireSession();
  ...
  const supabase = await createClient();
  const { chatId, error } = await startOrderChat(
    supabase,
    parsedId.data,
    user.id
  );
  if (error || !chatId)
    return { success: false, error: error ?? "Failed to start chat." };
