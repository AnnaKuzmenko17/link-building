...
import { buildCopywriterOrderColumns } from "./copywriter-orders-columns";
...
export function CopywriterOrdersClient({ orders, role }: Props) {
  const router = useRouter();
  const [filtersOpen, setFiltersOpen] = useState(false);
  const [filterStatus, setFilterStatus] = useState("");

  const columns = useMemo(() => buildCopywriterOrderColumns(role), [role]);
