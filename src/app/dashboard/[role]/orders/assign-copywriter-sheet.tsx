...
export function AssignCopywriterSheet({
  open,
  onOpenChange,
  order,
  copywriters,
  ...
  const allCopywriters =
    order.copywriter &&
    !copywriters.some((cw) => cw.id === order.copywriter!.id)
  ...
  async function handleSave() {
    ...
    toast.success(
      isReassign ? "Copywriter reassigned." : "Copywriter assigned."
    );
    onOpenChange(false);
    router.refresh();
  }
        ...
        <div className="flex flex-1 flex-col gap-4 px-4 py-4">
          <div className="flex flex-col gap-1.5">
            ...
            <Select
              ...
              items={allCopywriters.map((cw) => ({
                value: cw.id,
                label: `${cw.first_name} ${cw.last_name}`,
              }))}
            >
