...
import {
  ...
  setUserStatus,
  updateUserProfile,
  type ActiveCopywriter,
  type ActiveOrderForReassign,
} from "@/lib/data/users";
...
export async function editUserAction(input: {
  ...
  if (
    ...
    return {
      success: false,
      error: "Managers cannot edit admin or manager accounts.",
    };
  }

  if (
    viewerRole === "manager" &&
    !MANAGER_ASSIGNABLE_ROLES.includes(role as Role)
  ) {
  ...
  const { error } = await updateUserProfile(adminClient, targetId, {
    first_name,
    last_name,
    email,
    role: role as Role,
...
export async function resendInviteAction(targetId: string): Promise<Result> {
  ...
  if (!res.ok) {
    ...
    return {
      success: false,
      error: body?.message ?? "Failed to send invite email.",
    };
  }
...
export type DisablePreCheck =
  ...
  | { type: "sourcer" }
  | {
      type: "copywriter_reassign";
      orders: ActiveOrderForReassign[];
      copywriters: ActiveCopywriter[];
    }
  | { success: false; error: string };

export async function getDisablePreCheckAction(
  targetId: string
): Promise<DisablePreCheck> {
  const { role: viewerRole } = await requireSession();
  if (viewerRole !== "admin")
    return { success: false, error: "Not authorized." };
...
export async function disableUserAction(targetId: string): Promise<Result> {
  ...
  if (target.role === "admin")
    return { success: false, error: "Cannot disable another admin account." };

  const { error } = await setUserStatus(adminClient, targetId, "disabled");
  if (error)
    return {
      success: false,
      error: "Failed to disable user. Please try again.",
    };
  return { success: true };
}
...
export async function disableSourcerAction(targetId: string): Promise<Result> {
  ...
  const { error: sitesError } = await unlinkSourcerFromSites(
    adminClient,
    targetId
  );
  if (sitesError)
    return {
      success: false,
      error: "Failed to unlink sourcer from sites. Please try again.",
    };
...
export async function disableCopywriterWithReassignAction(input: {
  ...
  for (const { orderId, copywriterId } of assignments) {
    ...
    const { error } = await reassignOrder(
      adminClient,
      orderId,
      userId,
      copywriterId
    ...
    if (error)
      return {
        success: false,
        error: "Failed to reassign order. Please try again.",
      };
  }
...
export async function activateUserAction(targetId: string): Promise<Result> {
  ...
  if (target.role === "admin")
    return {
      success: false,
      error: "Cannot modify another admin account status.",
    };
...
export async function assignManagerAction(input: {
  ...
  if (managerUser.role !== "manager")
    return { success: false, error: "Selected user is not a manager." };
  if (managerUser.status !== "active")
    return { success: false, error: "Selected manager is not active." };
