...
import type { Invoice, InvoiceItem, InvoiceStatus, User, Order, Site } from '@/types'
...
export async function getInvoices(
  ...
  let query = supabase
    .from('invoices')
    .select('*, client:users!client_id(id, first_name, last_name, email), invoice_items(amount)')
...
export async function getPublishedOrdersNotInvoiced(
  ...
  return (data ?? []) as unknown as Array<{ id: string; client_id: string; created_at: string }>
}
