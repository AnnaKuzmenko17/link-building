...
const ACTIVE_ORDER_STATUSES = ['new', 'in_progress', 'content_sent', 'needs_changes', 'content_approved'] as const

const VALID_STATUS_VALUES = ['pending', 'active', 'disabled'] as const
type ValidStatus = typeof VALID_STATUS_VALUES[number]

function isValidRole(v: string): v is 'client' | 'manager' | 'copywriter' | 'sourcer' | 'admin' {
...
async function attachManagers(supabase: Client, users: User[]): Promise<UserWithManager[]> {
  ...
  const managerMap = new Map(
    (managers ?? []).map((m) => [m.id, { first_name: m.first_name, last_name: m.last_name }])
  )
...
export async function getUserById(supabase: Client, id: string): Promise<UserWithManager | null> {
