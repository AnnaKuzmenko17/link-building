...
async function getManagerMetrics(supabase: Client): Promise<MetricCard[]> {
  ...
  return [
    ...
    },
    {
      label: "Active Sites",
      value: activeSites ?? 0,
      href: "/dashboard/manager/sites",
    },
  ];
}

async function getCopywriterMetrics(
  supabase: Client,
  userId: string
): Promise<MetricCard[]> {
  ...
  return [
    ...
    {
      label: "Needs Changes",
      value: needsChanges ?? 0,
      href: "/dashboard/copywriter/orders",
    },
  ];
}

async function getSourcerMetrics(
  supabase: Client,
  userId: string
): Promise<MetricCard[]> {
  ...
  return [
    {
      label: "Active Sites",
      value: activeSites ?? 0,
      href: "/dashboard/sourcer/sites",
    },
    {
      label: "Pending Sites",
      value: pendingSites ?? 0,
      href: "/dashboard/sourcer/sites",
    },
  ];
}
...
async function getAdminMetrics(supabase: Client): Promise<MetricCard[]> {
  const [{ count: openOrders }, { count: totalUsers }, { count: activeSites }] =
    await Promise.all([
      supabase
        .from("orders")
        .select("*", { count: "exact", head: true })
...
export function getMetricsForRole(
  ...
  switch (role) {
    case "client":
      return getClientMetrics(supabase, userId);
    case "manager":
      return getManagerMetrics(supabase);
    case "copywriter":
      return getCopywriterMetrics(supabase, userId);
    case "sourcer":
      return getSourcerMetrics(supabase, userId);
    case "admin":
      return getAdminMetrics(supabase);
  }
}
