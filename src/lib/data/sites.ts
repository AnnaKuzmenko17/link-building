import type { Category, Role, Site, SiteStatus } from "@/types";
...
export async function getSites(
  ...
  let query = supabase
    .from("sites")
    .select(
      "*, category:categories(id, name), created_by_user:users!created_by(first_name, last_name)"
    )
  ...
  if (filters.link_type)
    query = query.eq("link_type", filters.link_type as Site["link_type"]);
  if (filters.price_from != null)
    query = query.gte("price", filters.price_from);
...
export async function getSiteById(
  ...
  const { data } = await supabase
    .from("sites")
    .select(
      "*, category:categories(id, name), created_by_user:users!created_by(first_name, last_name), approved_by_user:users!approved_by(first_name, last_name), needs_changes_by_user:users!needs_changes_by(first_name, last_name)"
    )
    .eq("id", id)
    .maybeSingle();
